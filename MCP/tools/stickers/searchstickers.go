...
func SearchstickersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/stickers/search%s", cfg.BaseURL, queryString)
...
func CreateSearchstickersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_stickers_search",
		mcp.WithDescription("Search Stickers"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    SearchstickersHandler(cfg),
	}
}
